...
('Qt 用户端：原生 C++17 实现',['Qt 6.5.3 Widgets；首页、找桩、详情、充电、统计与账户','沿用深紫卡片、车辆资产和能量光环，QPainter 原生绘制','验证码登录、钱包充值、预约启停、结算与历史订单','Web 用户端保留；桌面程序没有嵌入浏览器'], '根据总体架构要求新增原生Qt用户端。网络访问由C++异步请求完成，车辆和地图由Qt控件与自绘构成；服务器继续决定真实电量、费用和状态。原Web页面保留，不能把Web访问等同于运行Qt程序。','qt-user'),
('Qt 管理端：五页运营工作台',['C++17 / Qt Widgets；Qt Charts 只链接管理端','运营总览、站点导航、充电订单、操作审计、负荷预测','原生管理表单接入用户、设备、订单、资金与设置接口','Ubuntu 已编译；Windows 提供构建脚本，尚未运行验证'], '管理员桌面端复用后端权限和事务边界。Qt Charts展示环图和预测折线，QPainter重绘目标环、流带与地图。管理员所有写操作通过原API确认，不直接访问数据库。用户要求跳过后续完整验收，因此不能声称Qt完整业务验收已通过。','qt-admin'),
...
qas += [('Qt 是网页套壳吗？','不是。客户端由C++17和Qt Widgets构成，没有QWebEngine；Qt Charts仅管理端链接，网络和本地缓存分别使用Qt Network/WebSockets/SQL。'),('Qt 重构验收完成了吗？','两个Linux程序已编译。按用户要求跳过后续完整验收，Windows未运行；不得把既有Web和后端验收结果视为Qt完整验收。'),('模型真的训练了吗？','训练了按范围独立拟合的岭回归，保留系数、输入数据散列和复现脚本，同时对比昨日与上周周期基线。详见 docs/LOAD_FORECAST.md。'),('为什么岭回归测试更好却选择昨日基线？','模型按验证集选择。全网验证期昨日基线更好，不能在看到测试集后改选，否则测试不再独立。'),('这能预测今天的负荷吗？','不能。当前数据截止2023年2月，页面是次日历史预测实验。需要接入更新的实测电量后再验证上线。')]
