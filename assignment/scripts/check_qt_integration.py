...
async def main():
    ...
    with (ROOT/'.runtime/qt-migration/test-server.log').open('w') as log:
        try:
            ...
            command=[str(ROOT/'.runtime/qt-build/test-desktop'),'-platform','offscreen']
            if os.environ.get('ELECTRA_DEBUG_CRASH'):
                command=[str(ROOT/'.runtime/qt-deps/usr/bin/gdb'),'-batch','-ex','run','-ex','bt','--args',*command]
            subprocess.run(command,cwd=ROOT,env={**env,'LD_LIBRARY_PATH':str(ROOT/'.runtime/qt-deps/usr/lib/x86_64-linux-gnu'),'QT_QPA_PLATFORM':'offscreen','ELECTRA_TEST_API':base},check=True,timeout=150)
        finally:
            if server:
                server.terminate()
