...
for folder in ['bin','lib','plugins','licenses']:(LINUX/folder).mkdir(parents=True,exist_ok=True)
for name in ['electra-user','electra-admin']:
    shutil.copy2(ROOT/'.runtime/qt-build'/name,LINUX/'bin'/name)
...
queue=[*list((LINUX/'bin').iterdir()),*list((LINUX/'plugins').rglob('*.so'))]
seen=set()
while queue:
    binary=queue.pop()
...
for role in ['admin','user']:
    source=ROOT/'.runtime/qt-migration'/role
...
shutil.copy2(ROOT/'.runtime/qt-migration/final-build.log',OUT/'编译日志.txt')
(OUT/'README.txt').write_text('''Electra Qt 桌面端交付 · 2026-09-07
