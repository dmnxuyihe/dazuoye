...
class AdminService:
    ...
    async def create_station(
        ...
        *,
        ...
        async with self.pool.acquire() as connection, connection.transaction():
            ...
            await self._log(
                connection, admin_id, "station.created", "station", station_id
            )
            await emit_event(
                connection,
                "station.changed",
        ...
        row = await self.pool.fetchrow(
            ...
            SELECT
              ...
              (SELECT COALESCE(sum(amount), 0) FROM charging_order
                 WHERE status = 'completed' AND ended_at::date = CURRENT_DATE) AS today_revenue,
              (SELECT count(*) FROM charging_order
                 WHERE status = 'completed' AND ended_at::date = CURRENT_DATE) AS today_orders,
              (SELECT COALESCE(sum(amount), 0) FROM charging_order
                 WHERE status = 'completed' AND date_trunc('month', ended_at) = date_trunc('month', now())) AS month_revenue
            """
        )
        return dict(row)
    ...
    async def revenue(self, days: int) -> list[dict[str, Any]]:
        rows = await self.pool.fetch(
            ...
            LEFT JOIN charging_order o ON o.status = 'completed' AND o.ended_at::date = day::date
