  ...
  chargeTimer = setInterval(async () => {
    ...
    try {
      ...
      $("#charge-state").textContent =
        activeOrder.status === "completed" ? "充电已完成" : "正在充电";
...
async function stopCharge() {
  ...
  try {
    activeOrder = await api(`/orders/${activeOrder.id}/stop`, {
      method: "POST",
    });
    clearInterval(chargeTimer);
