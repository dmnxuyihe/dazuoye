...
See [Qt migration and delivery](../docs/QT_MIGRATION.md) for scope, API/cache design, launch/build instructions and current verification evidence.
...
Only the map uses Qt WebEngine + Leaflet + online OSM tiles; other pages and business actions remain native C++ / Qt. See [online map](../docs/QT_ONLINE_MAP.md). Web clients and FastAPI remain in the parent project.

## Native page reconstruction

See [current page mapping and evidence](../docs/QT_MIGRATION.md). `src/visuals.*` implements the native segmented controls and data graphics. `assets/web-ui.json` contains existing Web icon paths/labels, rendered by Qt SVG.
