...
StationRecommendations::StationRecommendations(QWidget *parent) : QWidget(parent) {
    ...
    detail->setStyleSheet(
        "QFrame#station-expanded-card{background:#251733;border:1px solid #50305f;"
        "border-radius:14px;} QFrame#metric-cell{border:0;background:transparent;}"
        "QFrame#metric-divider{background:#50305f;border:0;}");
    detailLayout = new QVBoxLayout(detail);
...
void StationRecommendations::rebuild() {
    ...
    summary->setText(selectedStation.isEmpty() ? "选择充电站"
        : QString("%1    %2    空闲 %3/%4    %5 元/kWh    %6")
            ...
            .arg(expanded ? "⌃" : "⌄"));
    detail->setVisible(expanded && !selectedStation.isEmpty());
    scroll->setVisible(expanded && rows.size() > 1);
    if (!selectedStation.isEmpty()) {
        detailLayout->addWidget(label(text(selectedStation, "address", "暂无详细地址"),
        ...
        auto addMetric = [&](const QString &caption, const QString &value) {
            auto cell = new QFrame;
            cell->setObjectName("metric-cell");
            auto layout = new QVBoxLayout(cell);
            layout->setContentsMargins(4, 0, 4, 0);
            layout->setSpacing(5);
            layout->addWidget(label(caption, "color:#a991ba;font-size:10px;"), 0, Qt::AlignHCenter);
            layout->addWidget(label(value, "color:#fff4ff;font-size:16px;font-weight:600;"), 0, Qt::AlignHCenter);
            metricRow->addWidget(cell, 1);
        };
        auto divider = [&] {
            auto line = new QFrame;
            line->setObjectName("metric-divider");
            line->setFixedWidth(1);
            metricRow->addWidget(line);
        };
        addMetric("距离", distanceText(selectedStation));
        divider();
        addMetric("空闲电桩", QString("%1/%2").arg(qRound(number(selectedStation, "available_count")))
                                                  .arg(qRound(number(selectedStation, "charger_count"))));
        divider();
        addMetric("基础价格", money(number(selectedStation, "unit_price")) + " 元/kWh");
        ...
        const auto id = text(selectedStation, "id");
