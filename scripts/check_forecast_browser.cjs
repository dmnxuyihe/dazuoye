...
(async()=>{
 ...
 await page.goto((process.env.CHARGING_TEST_API || 'http://127.0.0.1:4173') + '/ui/admin.html#forecast');
 await page.locator('.forecast-chart').waitFor();
 await page.evaluate(()=>document.fonts.ready);
 assert.equal(await page.locator('#forecast-scope option').count(),14);
 ...
 await page.waitForFunction(() => document.querySelector('.forecast-kpis').textContent.includes(document.querySelector('#forecast-scope').selectedOptions[0].textContent));
 ...
 console.log('PASS: forecast deep link, 14 scopes, region change, historical date, responsive 390/768/1440/1920, navigation, no browser errors.');
