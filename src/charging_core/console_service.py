...
class ConsoleService(AdminService):
    ...
    async def money(self, admin, user_id, amount, key, reason, order_id=None):
        async with self.pool.acquire() as c, c.transaction():
            ...
            if amount < 0 and await c.fetchval(
                "SELECT EXISTS(SELECT 1 FROM charging_order WHERE user_id=$1 AND status IN ('reserved','charging','pending_payment'))",
                user_id,
            ):
                raise ConflictError("请先结束活动订单，再进行负向调整")
            balance = user["balance"] + amount
            if balance < user['held_balance']:
                raise ConflictError("调整后余额不能低于已冻结金额")
    ...
    async def withdrawals(self, status=None):
        return [dict(row) for row in await self.pool.fetch(
            """SELECT w.*,u.phone,u.nickname FROM withdrawal_request w JOIN app_user u ON u.id=w.user_id
               WHERE ($1::text IS NULL OR w.status=$1) ORDER BY w.requested_at DESC""",status)]

    async def review_withdrawal(self, admin_id, request_id, approve, note):
        async with self.pool.acquire() as c, c.transaction():
            user_id=await c.fetchval("SELECT user_id FROM withdrawal_request WHERE id=$1",request_id)
            if not user_id: raise NotFoundError("提现申请不存在")
            user=await c.fetchrow("SELECT * FROM app_user WHERE id=$1 FOR UPDATE",user_id)
            request=await c.fetchrow("SELECT * FROM withdrawal_request WHERE id=$1 FOR UPDATE",request_id)
            status="paid" if approve else "rejected"
            if request["status"] != "pending":
                if request["status"] != status: raise ConflictError("该申请已处理，不能更改结果")
                return dict(request)
            amount=request["amount"]
            balance=user["balance"]-amount if approve else user["balance"]
            await c.execute("UPDATE app_user SET balance=$2,held_balance=held_balance-$3 WHERE id=$1",user_id,balance,amount)
            if approve:
                await c.execute("""INSERT INTO wallet_entry(id,user_id,idempotency_key,entry_type,amount,balance_after)
                    VALUES($1,$2,$3,'withdrawal',$4,$5)""",uuid4(),user_id,request_id,-amount,balance)
            row=await c.fetchrow("""UPDATE withdrawal_request SET status=$2,reviewer_id=$3,review_note=$4,decided_at=now() WHERE id=$1 RETURNING *""",
                                request_id,status,admin_id,note)
            await self._log(c,admin_id,"withdrawal."+status,"withdrawal",request_id,{"amount":str(amount),"note":note,"mode":"simulation"})
            await emit_event(c,"wallet.withdrawal_reviewed",{"request_id":request_id,"status":status},audience_user_id=user_id,audience_role="admin")
            return dict(row)
