...
ALTER TABLE wallet_entry ADD CONSTRAINT wallet_entry_entry_type_check CHECK (entry_type IN ('recharge','charge','adjustment','refund','withdrawal'));

-- Optional simulation target; NULL preserves older clients' charging behaviour.
ALTER TABLE charging_order ADD COLUMN IF NOT EXISTS target_energy_kwh numeric(12,6) CHECK (target_energy_kwh > 0);
ALTER TABLE charging_order ADD COLUMN IF NOT EXISTS initial_soc numeric(5,2) CHECK (initial_soc BETWEEN 0 AND 100);
ALTER TABLE charging_order ADD COLUMN IF NOT EXISTS battery_kwh numeric(8,2) CHECK (battery_kwh > 0);
...
-- Completion workflow: server profiles, frozen funds, immutable tariffs, explicit payment.
ALTER TABLE app_user ADD COLUMN IF NOT EXISTS held_balance numeric(12,2) NOT NULL DEFAULT 0;
ALTER TABLE app_user ADD COLUMN IF NOT EXISTS vehicle_name varchar(80) NOT NULL DEFAULT '';
ALTER TABLE app_user ADD COLUMN IF NOT EXISTS vehicle_plate varchar(20) NOT NULL DEFAULT '';
ALTER TABLE app_user ADD COLUMN IF NOT EXISTS vehicle_soc numeric(5,2) NOT NULL DEFAULT 40 CHECK(vehicle_soc BETWEEN 0 AND 100);
ALTER TABLE app_user ADD COLUMN IF NOT EXISTS battery_kwh numeric(8,2) NOT NULL DEFAULT 60 CHECK(battery_kwh > 0 AND battery_kwh <= 300);
ALTER TABLE app_user ADD COLUMN IF NOT EXISTS charge_limit numeric(5,2) NOT NULL DEFAULT 80 CHECK(charge_limit > 0 AND charge_limit <= 100);
ALTER TABLE app_user ADD COLUMN IF NOT EXISTS avatar_data text;
CREATE UNIQUE INDEX IF NOT EXISTS uq_vehicle_plate ON app_user(vehicle_plate) WHERE vehicle_plate <> '';
ALTER TABLE station ADD COLUMN IF NOT EXISTS tariff jsonb NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE station ADD COLUMN IF NOT EXISTS address_verified boolean NOT NULL DEFAULT false;
ALTER TABLE charging_order ADD COLUMN IF NOT EXISTS tariff_snapshot jsonb NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE charging_order ADD COLUMN IF NOT EXISTS billing_detail jsonb NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE charging_order ADD COLUMN IF NOT EXISTS vehicle_name varchar(80) NOT NULL DEFAULT '';
ALTER TABLE charging_order ADD COLUMN IF NOT EXISTS vehicle_plate varchar(20) NOT NULL DEFAULT '';
ALTER TABLE charging_order ADD COLUMN IF NOT EXISTS paid_at timestamptz;
DO $$ DECLARE item record; BEGIN
    FOR item IN SELECT conname FROM pg_constraint WHERE conrelid='charging_order'::regclass
        AND contype='c' AND pg_get_constraintdef(oid) LIKE '%status%' LOOP
        EXECUTE format('ALTER TABLE charging_order DROP CONSTRAINT %I', item.conname);
    END LOOP;
END $$;
ALTER TABLE charging_order ADD CONSTRAINT charging_order_status_check CHECK(status IN ('reserved','charging','pending_payment','completed','cancelled'));
ALTER TABLE charging_order ADD CONSTRAINT charging_order_lifecycle_check CHECK(
    (status='reserved' AND started_at IS NULL AND ended_at IS NULL)
    OR (status='charging' AND started_at IS NOT NULL AND ended_at IS NULL)
    OR (status IN ('pending_payment','completed','cancelled') AND ended_at IS NOT NULL));
DROP INDEX IF EXISTS uq_active_order_per_user;
CREATE UNIQUE INDEX uq_active_order_per_user ON charging_order(user_id) WHERE status IN ('reserved','charging','pending_payment');
ALTER TABLE app_user DROP CONSTRAINT IF EXISTS app_user_held_balance_check;
ALTER TABLE app_user ADD CONSTRAINT app_user_held_balance_check CHECK(held_balance >= 0 AND held_balance <= balance);
CREATE TABLE IF NOT EXISTS withdrawal_request (
    id uuid PRIMARY KEY, user_id uuid NOT NULL REFERENCES app_user(id),
    request_key uuid NOT NULL, amount numeric(12,2) NOT NULL CHECK(amount > 0),
    status varchar(16) NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','paid','rejected')),
    destination varchar(100) NOT NULL DEFAULT '演示钱包',
    requested_at timestamptz NOT NULL DEFAULT now(), decided_at timestamptz,
    reviewer_id uuid REFERENCES admin_account(id), review_note varchar(200),
    UNIQUE(user_id,request_key)
);
